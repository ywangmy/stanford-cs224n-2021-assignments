...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.combined_projection = nn.Linear(in_features=3*self.hidden_size, out_features=self.hidden_size, bias=False)
        self.target_vocab_projection = nn.Linear(in_features=self.hidden_size, out_features=len(self.vocab.tgt), bias=False)
        self.dropout = nn.Dropout(self.dropout_rate) # Inplace ?
        ### END YOUR CODE
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # (1)
        X = self.model_embeddings.source( torch.tensor([[ d2 for d2 in d1 ] for d1 in source_padded], device=self.device) )
        assert X.shape == torch.Size([source_padded.shape[0], source_padded.shape[1], self.model_embeddings.embed_size]), "X\'s shape incorrect"
        # (2)
        X = nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths, enforce_sorted=True)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(input=X) # Default initial h and c are zeros
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(sequence=enc_hiddens)
        enc_hiddens = torch.permute(input=enc_hiddens, dims=(1, 0, 2))
        # (3)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # (2)
        Y = self.model_embeddings.target( torch.tensor([[ d2 for d2 in d1 ] for d1 in target_padded], device=self.device) )
        # (3)
        ...
        # print("TEST 1", combined_outputs.get_device(), torch.tensor([0, 1], device=self.device).get_device(), torch.tensor([0, 1]).get_device(), self.device)
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # (3)
        mat2 = torch.unsqueeze(input=dec_hidden, dim=2) # (b, h, 1)
        e_t = torch.squeeze( input=torch.bmm(input=enc_hiddens_proj, mat2=mat2), dim=2 ) # (b, src_len)
        ...
        # (2)
        mat1 = torch.unsqueeze(input=alpha_t, dim=1) # (b, 1, src_len)
        a_t = torch.squeeze( input=torch.bmm(input=mat1, mat2=enc_hiddens), dim=1 ) # (b, 1, 2h) -> (b, 2h)
        # (3)
        U_t = torch.cat(tensors=(dec_hidden, a_t), dim=1) # (b, 3h)
        # (4)
        V_t = self.combined_projection(U_t) # (b, h)
        # (5)
