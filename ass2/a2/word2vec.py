...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    P = softmax(np.dot(outsideVectors, np.reshape(centerWordVec, (-1, 1))).flatten()).reshape(-1, 1) # (N, 1)
    loss = -np.log(P[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.dot(P.T, outsideVectors).flatten() # (d, )
    gradOutsideVecs = np.dot(P, np.reshape(centerWordVec, (1, -1))) # (N, d)
...
def negSamplingLossAndGradient(
    ...
    centerWordVec_d1 = np.reshape(centerWordVec, (-1, 1)) # (d, 1)
    loss = -np.log(sigmoid( np.dot(outsideVectors[outsideWordIdx], centerWordVec_d1) ))
    for s in negSampleWordIndices:
        loss -= np.log(sigmoid( -np.dot(outsideVectors[s], centerWordVec_d1) ))
    dot_products = np.array([ np.dot(outsideVectors[i].T, centerWordVec_d1) for i in indices ])
    reuse = np.array([ -sigmoid(dot_products[0]) * np.exp(-dot_products[0]) ] + [ sigmoid(-dot_products[i]) * np.exp(dot_products[i]) for i in range(1, K+1) ]) # (K+1, 1)
    U = np.array([ outsideVectors[i] for i in indices ]) # (K+1, d)
    gradCenterVec = np.dot(U.T, reuse).flatten() # (d, )
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradSamples = np.dot(reuse, centerWordVec_d1.T) # (K+1, d)
    for i, idx in enumerate(indices):
        gradOutsideVecs[idx] += gradSamples[i]
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    num = len(outsideVectors)
    centerWordIdx = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[centerWordIdx].flatten() # (d, )
    outsideWordIdxs = [word2Ind[o] for o in outsideWords]
    for i in outsideWordIdxs:
        tmp_loss, tmp_gradC, tmp_gradO = word2vecLossAndGradient(centerWordVec, i, outsideVectors, dataset)
        loss += tmp_loss
        gradCenterVecs[centerWordIdx] += tmp_gradC
