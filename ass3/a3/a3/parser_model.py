...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # (1)
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros([self.n_features * self.embed_size, self.hidden_size]), requires_grad=True) # W
        nn.init.xavier_uniform_(self.embed_to_hidden_weight, gain=nn.init.calculate_gain('relu'))
        
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros([self.hidden_size]), requires_grad=True) # b1
        ...
        # (3)
        self.hidden_to_logits_weight = nn.Parameter(torch.zeros([self.hidden_size, self.n_classes]), requires_grad=True) # U
        nn.init.xavier_uniform_(self.embed_to_hidden_weight, gain=nn.init.calculate_gain('relu'))
        
        self.hidden_to_logits_bias = nn.Parameter(torch.zeros([self.n_classes]), requires_grad=True) # b2
        nn.init.uniform_(self.embed_to_hidden_weight)

        #print("Trainable", self.embed_to_hidden_weight)
        ### END YOUR CODE
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.flatten(torch.index_select(self.embeddings, 0, torch.tensor([ feature for batch in w for feature in batch ]))).view(w.shape[0], -1)
        """
        print(x)
        for i, batch in enumerate(x):
            tmp = torch.flatten((torch.index_select(self.embeddings, 0, torch.tensor([ feature for feature in w[i] ]))))
            print(tmp)
            print(batch)
            print(torch.equal(batch, tmp))
        """
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = ReLU( torch.add(torch.matmul(x, self.embed_to_hidden_weight), self.embed_to_hidden_bias) ) # (batch_size, hidden_size)
        h = self.dropout(h)
        logits = torch.add( torch.matmul(h, self.hidden_to_logits_weight), self.hidden_to_logits_bias ) # (batch_size, n_classes)
        ### END YOUR CODE
        return logits
