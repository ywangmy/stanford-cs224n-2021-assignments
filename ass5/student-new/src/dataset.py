...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # (1)
        len_doc = len(doc)
        len_truncate = min(len_doc,random.randint(4, int(self.block_size * 7 / 8)))
        truncated = doc[:len_truncate]
        # (2)
        len_masked = random.randint(int(len_truncate * 3 / 16), int(len_truncate * 5 / 16))
        assert len_truncate - len_masked - 1 >= 1, (len_doc, len_truncate, len_masked)
        len_prefix = random.randint(1, len_truncate - len_masked - 1)
        prefix = truncated[:len_prefix]
        masked_content = truncated[len_prefix : len_prefix+len_masked]
        suffix = truncated[len_prefix+len_masked : ]
        # (3)
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (self.block_size - len_truncate - 2)
        # (4)
        inp = masked_string[:-1]
        oup = masked_string[1:]
