...
if args.function == 'pretrain':
    ...
    #         num_workers=4
    # [part f]
    finetune_name_dataset = dataset.NameDataset(pretrain_dataset, open(args.finetune_corpus_path, 'r').read())
    ...
    # [part c]
        ...
        tconf = trainer.TrainerConfig(max_epochs=75,
                                      ...
                                      final_tokens=200*len(pretrain_dataset)*block_size,
                                      num_workers=4)
    trainer = trainer.Trainer(model=model, train_dataset=finetune_name_dataset, test_dataset=None, config=tconf)
    trainer.train()
    torch.save(model.state_dict(), args.writing_params_path)
